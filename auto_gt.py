	...
	while day < 90 :
		...
		for ep in l :
			...
			if 'perfil_completo' in custom_fields:
				if custom_fields['perfil_completo'] != 'yes' or not is_profile_complete(custom_fields['expa_id'],ep['contactId']):
					continue			
			#check if the ep is an apllicant in gr and check if the ep is applicant in expa
			if 'aplicante' in custom_fields:
				if custom_fields['aplicante'] != 'yes' or not is_applicant(custom_fields['expa_id'],ep['contactId']):
...
def is_profile_complete(expa_id,gr_id,):
	...
	if len(ep['missing_profile_fields']):
